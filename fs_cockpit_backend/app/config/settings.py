...
class Settings(BaseSettings):
    ...
    INTUNE_GRAPH_URL: str = Field(
        default="https://graph.microsoft.com/v1.0", env="INTUNE_GRAPH_URL"
    )
    ...
    NEXTTHINK_GRANT_TYPE: str = Field(
        default="client_credentials", env=("NEXTTHINK_GRANT_TYPE", "NEXTHINK_GRANT_TYPE")
    )
    NEXTTHINK_SCOPE: str = Field(
        default="service:integration", env=("NEXTTHINK_SCOPE", "NEXTHIK_SCOPE")
    )
    ...
    AZURE_AD_TENANT_ID: str = Field(
        default="e168d6cf-6597-4aeb-ae6f-111c78a48f78", env="AZURE_AD_TENANT_ID"
    )
    AZURE_AD_CLIENT_ID: str = Field(
        default="64db8b2f-22ad-4ded-86b9-c91a43623f78", env="AZURE_AD_CLIENT_ID"
    )

    # Azure AD B2C specific settings
    AZURE_AD_B2C_DOMAIN: Optional[str] = Field(
        default=None, env="AZURE_AD_B2C_DOMAIN"
    )  # e.g., "zenpoc.b2clogin.com"
    AZURE_AD_B2C_POLICY: Optional[str] = Field(
        default=None, env="AZURE_AD_B2C_POLICY"
    )  # e.g., "B2C_1_NTT_SIGNUP_SIGNIN"
    AZURE_AD_VERIFY_SSL: bool = Field(
        default=True, env="AZURE_AD_VERIFY_SSL"
    ...
    def AZURE_AD_JWKS_URI(self) -> str:
        ...
        if self.AZURE_AD_B2C_DOMAIN and self.AZURE_AD_B2C_POLICY:
            ...
            return (
                f"https://login.microsoftonline.com/{self.AZURE_AD_TENANT_ID}/discovery/v2.0/keys"
            )
    ...
    AUTH_EXCLUDED_PATHS: List[str] = ["/health", "/docs", "/openapi.json", "/redoc", "/favicon.ico"]
    ...
    CACHE_TTL_DIAGNOSTICS: int = 600  # 10 minutes for device diagnostics
