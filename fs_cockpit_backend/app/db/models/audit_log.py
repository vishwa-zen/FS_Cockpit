...
class AuditLog(Base):
    ...
    # WHAT - Type of action
    action = Column(String(255), nullable=False, index=True)  # e.g., "view_incident", "generate_solution", "execute_action"
    
    # WHERE - Resource type affected
    resource_type = Column(String(100), nullable=True, index=True)  # e.g., "incident", "device", "remote_action"
    
    # WHICH - Specific resource ID
    resource_id = Column(String(100), nullable=True, index=True)  # e.g., incident_number, device_name, action_id
