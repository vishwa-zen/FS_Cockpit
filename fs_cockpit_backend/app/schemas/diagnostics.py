...
class OSBuildInfo(BaseModel):
    ...
    os_name: Optional[str] = Field(None, description="Operating system name (e.g., Windows 11 Enterprise)")
...
class SystemUptimeInfo(BaseModel):
    ...
    last_full_boot_duration_minutes: Optional[float] = Field(None, description="Last full boot duration in minutes")
...
class ComprehensiveDiagnosticsResponse(BaseModel):
    ...
    application_health: Optional[Dict[str, Any]] = Field(
        None,
        description="Application crash count and health metrics"
    )
    
    # Alerts and notifications
    alert_summary: Optional[Dict[str, Any]] = Field(
        None,
        description="Summary of active alerts and their severity"
    )
    
    # Raw query responses (for reference/debugging)
    raw_responses: Optional[Dict[str, Any]] = Field(
        None,
        description="Raw NQL query responses from NextThink"
    )
    
    # Metadata
    diagnostics_mode: str = Field(..., description="'full' or 'partial'")
    categories_available: List[str] = Field(default_factory=list, description="List of categories retrieved")
    categories_requested: List[str] = Field(default_factory=list, description="List of categories requested")
    data_completeness_percent: float = Field(..., description="Percentage of available data retrieved")
...
class DiagnosticsRequest(BaseModel):
    ...
    include_raw_responses: bool = Field(False, description="Whether to include raw NQL responses (for debugging)")
