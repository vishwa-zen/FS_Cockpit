...
class NextThinkDiagnosticsService:
    ...
    async def get_device_diagnostics(
        self, request: DiagnosticsRequest
    ) -> ComprehensiveDiagnosticsResponse:
        """
        ...
        # Build cache key based on device name, mode, and categories
        categories_key = "_".join(sorted(requested_categories)) if requested_categories else "all"
        cache_key = f"nextthink:diagnostics:{device_name}:{mode}:{categories_key}"
        ...
        if self.cache:
            ...
            if cached_response is not None:
                logger.debug(
                    "Cache hit for device diagnostics",
                    device_name=device_name,
                    cache_key=cache_key,
                )
        ...
        if mode == "full":
            ...
            if (
                "hardware" in categories_to_fetch
                and "hardware_performance" not in categories_to_fetch
            ):
            ...
            categories_to_fetch = [
                c for c in requested_categories if c in AVAILABLE_DIAGNOSTIC_CATEGORIES
            ]
            # Always include hardware_performance when hardware is requested
            if (
                "hardware" in categories_to_fetch
        ...
        try:
            async with NextThinkClient(
                ...
                raw_diagnostics = await client.get_device_diagnostics(
                    device_name=device_name, include_categories=categories_to_fetch
                )
            ...
            if categories_to_fetch:
                completeness = (
                    len(raw_diagnostics.get("queries_executed", []))
            ...
            # Cache the response
            if self.cache:
                self.cache.set(cache_key, response, ttl_seconds=self.settings.CACHE_TTL_DIAGNOSTICS)
                logger.debug(
                    "Cached device diagnostics",
                    device_name=device_name,
                    cache_key=cache_key,
                    ttl_seconds=self.settings.CACHE_TTL_DIAGNOSTICS,
                )
            ...
            logger.error(
                "Unexpected error retrieving diagnostics", device_name=device_name, error=str(e)
            )
    ...
    def _parse_os_info_from_device_info(
        self, raw_data: Dict[str, Any]
    ) -> Optional[OSHealthDiagnostics]:
        ...
        try:
            ...
            build_info = OSBuildInfo(
                ...
                days_since_last_update=self._safe_int(
                    row_dict.get("device.operating_system.days_since_last_update")
                ),
            )
            os_health.build_info = build_info

            # Uptime info
            days_since_last_boot = self._safe_int(
                row_dict.get("device.boot.days_since_last_full_boot")
            )
    ...
    def _parse_device_performance(self, raw_data: Dict[str, Any]) -> Optional[HardwareDiagnostics]:
        ...
        try:
            ...
            if cpu_avg is not None:
                hardware.cpu = CPUDiagnostics(cpu_usage_percent=cpu_avg, cpu_usage_24h_avg=cpu_avg)
            ...
            if gpu_avg is not None:
                ...
                hardware.gpu = GPUDiagnostics(gpu_usage_percent=gpu_avg, gpu_usage_24h_avg=gpu_avg)
            ...
            hardware.cpu.boot_metrics = {
                ...
                "average_boot_duration_7d_minutes": self._safe_float(
                    row_dict.get("device.average_boot_duration_7d")
                ),
                "average_time_until_desktop_ready_7d_seconds": self._safe_float(
                    row_dict.get("device.average_time_until_desktop_is_ready_7d")
                ),
            }

            # Network metrics
            wifi_signal = self._safe_float(row_dict.get("device.average_wifi_signal_strength_24h"))
            hardware.network_metrics = {"wifi_signal_strength_24h_percent": wifi_signal}
    ...
    def _parse_device_score(self, raw_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            scores = {
                ...
                "logon_speed_score": self._safe_float(
                    row_dict.get("device.last_logon_speed_score")
                ),
                "applications_score": self._safe_float(
                    row_dict.get("device.last_applications_score")
                ),
                "collaboration_score": self._safe_float(
                    row_dict.get("device.last_collaboration_score")
                ),
                "os_activation_score": self._safe_float(
                    row_dict.get("device.last_os_activation_score")
                ),
                "network_quality_score": self._safe_float(
                    row_dict.get("device.last_network_quality_score")
                ),
                "device_performance_score": self._safe_float(
                    row_dict.get("device.last_device_performance_score")
                ),
                "device_reliability_score": self._safe_float(
                    row_dict.get("device.last_device_reliability_score")
                ),
            }
    ...
    def _parse_alerts(self, raw_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {"alert_count": self._safe_int(row_dict.get("no_of_alerts"))}
