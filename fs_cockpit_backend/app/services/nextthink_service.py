...
class NextThinkService:
    ...
    def _apply_filters(
        ...
        if status_filter:
            ...
            filtered = [dto for dto in filtered if dto.status and dto.status.lower() in status_lower]
        ...
        filtered.sort(
            key=lambda x: datetime.strptime(x.updatedAt, "%Y-%m-%d %H:%M:%S") if x.updatedAt else datetime.min,
            reverse=True
        )
    ...
    async def health_check(self) -> dict:
        ...
        """
        logger.debug("Performing NextThink health check", auth_url=self.auth_base_url, api_url=self.api_base_url)
    ...
    async def authenticate(self) -> dict:
        ...
        """
        logger.debug("Authenticating with NextThink", auth_url=self.auth_base_url, api_url=self.api_base_url)
    ...
    async def get_remote_actions(
        ...
        logger.debug("Raw NextThink response", response_keys=raw.keys() if isinstance(raw, dict) else "not_dict")
        ...
        if isinstance(raw, dict):
            ...
            logger.debug("NextThink data structure", num_rows=len(data_rows), num_headers=len(headers))
            ...
            for row in data_rows:
                if isinstance(row, list) and len(row) == len(headers):
                    ...
                    logger.warning("Row length mismatch", row_length=len(row) if isinstance(row, list) else "not_list", headers_length=len(headers))
    ...
    def _score_action_by_category(self, action: RemoteActionDTO, category: str, description: str) -> float:
        """
        ...
        if category_lower == "hardware":
            ...
            if any(kw in action_name for kw in ["hardware", "health", "diagnostic", "disk", "memory", "cpu"]):
                score += 40
            ...
            if "vpn" in description_lower or "network" in description_lower:
                if any(kw in action_name for kw in ["vpn", "network", "connectivity", "ping", "dns"]):
                    score += 50
            if "software" in description_lower or "app" in description_lower or "application" in description_lower:
                if any(kw in action_name for kw in ["software", "app", "install", "update", "patch"]):
                    score += 50
            ...
            if any(kw in action_name for kw in ["software", "application", "app", "install", "update", "patch"]):
                score += 40
                
        elif category_lower == "network":
            # Network issues: prioritize network diagnostics
            if any(kw in action_name for kw in ["network", "vpn", "connectivity", "ping", "dns", "proxy"]):
                score += 40
        ...
        for action in all_actions:
            ...
            if score > 0:  # Only include actions with positive scores
                ...
                if action_name and action_name not in seen_action_names:
                    ...
                    existing_idx = next((i for i, (s, a) in enumerate(scored_actions) 
                                        if a.actionName == action_name), None)
